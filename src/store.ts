...
export function createStore(
  ...
  return {
    ...
    createPage(slug, page, locale = defaultLocale) {
      if (adapter.exists(slug, locale)) throw new Error(`meditor: page "${slug}" already exists`);
    ...
    duplicatePage(slug, locale = defaultLocale) {
      ...
      if (published === null && draft === null) throw new Error(`meditor: page "${slug}" does not exist`);
