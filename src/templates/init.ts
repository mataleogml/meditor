// Template source for `meditor init` (see ../cli.ts). Kept as plain string
...
// cms.config.ts. `meditor apply-settings` (see ../cli.ts) greps a host's
...
export const MEDITOR_REWRITE_MARKER = "meditor:admin-route-rewrite";
export const MEDITOR_BOOTSTRAP_MARKER = "meditor:bootstrap-wired";

const meditorConfigJson = `{
  "adminRoute": "editor",
  "locales": ["en"],
  "defaultLocale": "en",
...
// ${MEDITOR_REWRITE_MARKER} — \`meditor apply-settings\` looks for this marker
...
// ${MEDITOR_BOOTSTRAP_MARKER} — adminRoute/locales/defaultLocale/routing live in
// meditor.config.json at the project root (edit via Settings → Advanced, or by
// hand); read once here so they parameterize the adapter below. \`meditor
...
/** The one object that wires meditor into this project. */
export const cmsConfig: CmsConfig = {
  ...
  // "meditor/auth/next-auth" or "meditor/auth/clerk" — no other change needed.
  auth: localAuth(),
...
// (brand/SEO/theme) is a reserved-slug doc in a private \`.meditor/\` dir, the
...
export async function writeBootstrap(patch: Partial<SiteSettingsBootstrap>) {
  if (!(await resolveAuth(cmsConfig).authorize({ action: "admin" }))) throw new Error("meditor: unauthorized");
...
import type { MediaAsset, Messages, PageContent, SaveResult } from "meditor";
import type {
  MediaSectionProps,
  PagesSectionProps,
  ResolvedSection,
  ...
  SiteSettingsRuntime,
} from "meditor/element";

/**
 * Mounts \`<meditor-shell>\`, the admin shell.
 *
...
export default function EditorShellClient({
  ...
  useEffect(() => {
    let cancelled = false;
    // Client-only: importing "meditor/element" is what defines the meditor-*
    // custom elements. Dynamic so \`customElements.define\` never runs on the
    // server render.
    import("meditor/element").then(() => {
  ...
  const ctx = useMemo<SectionCtx>(() => {
    ...
    return {
      ...
      // Replaced by the shell with one that reaches its own \`<meditor-top-bar>\`.
      setTopBarAction: () => {},
  ...
  return registered ? (
    ...
    <meditor-shell ref={ref} style={{ display: "flex", position: "fixed", inset: 0 }} />
  ) : null;
}
`;

const editorRoutePage = `import { notFound } from "next/navigation";
import { resolveAuth, resolveSections, type SiteSettings } from "meditor";
...
export default async function EditorRoute({
  ...
  if (active.kind === "pages") {
    ...
    // fields stay undefined and the shell renders \`<meditor-page-list>\`.
...
import { initPreviewBridge } from "meditor/element";
...
// wrapped with data-scms-index so it's click-selectable on the canvas.
export const dynamic = "force-dynamic";
...
const editorMediaUploadRoute = `import { handleMediaUpload } from "meditor";
import { cmsConfig } from "@/cms.config";
...
const slicesFieldSchema = `import type { SliceSchema } from "meditor";
...
const slicesSliceZone = `import type { Slice } from "meditor";
...
/** Every file \`meditor init\` scaffolds, keyed by path relative to the
