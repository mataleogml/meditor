...
import type { ContentAdapter, MediaAdapter, SliceSchema } from "./types";
import type { PageActions } from "./actions";
import type { Messages } from "./i18n";
import type { FieldFormMedia } from "./ui/image-picker-field.element";
...
export interface CustomSection extends SectionBase {
   ...
   *  that its renderer lives in the host. The Lit shell can't render these and
   *  says so loudly rather than painting an empty pane. */
  host?: true;
}
...
export function resolveSections(config: { sections?: Section[] }): ResolvedSection[] {
  ...
  return src.map((s) => {
    if (s.kind === "custom") {
      ...
      if (provided !== 1)
        throw new Error(
          `meditor: custom section "${s.label}" needs exactly one of element/mount/host (got ${provided})`
        );
    }
    const id = s.id ?? (s.kind === "collection" || s.kind === "custom" ? slug(s.label) : s.kind);
    if (seen.has(id)) throw new Error(`meditor: duplicate section id "${id}"`);
    seen.add(id);
