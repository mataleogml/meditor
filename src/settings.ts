...
export interface SiteSettingsRuntime {
  ...
  theme?: { preset?: "shadcn" | "fdn" | "atlas" | "neutral" };
}
...
export function createSettingsStore(config: CmsConfig): SettingsStore {
  const dir = path.join(path.dirname(config.adapter.root), ".meditor");
  ...
  const guardAdmin = async () => {
    if (!(await auth.authorize({ action: "admin" }))) throw new Error("meditor: unauthorized");
  };
  ...
  return {
    ...
    bootstrap: config.settingsAdapter ?? createFsSettingsAdapter({ file: path.join(process.cwd(), "meditor.config.json") }),
  };
}
