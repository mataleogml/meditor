...
beforeEach(() => {
  dir = fs.mkdtempSync(path.join(os.tmpdir(), "meditor-cli-"));
});
...
describe("scaffold", () => {
  ...
  // The scaffold ships the SDUI admin shell, not the superseded React 3-pane
  // editor. Two things have to stay true: nothing imports the React entry, and
  // the optional catch-all is the only `page` directly under app/editor (a
  // sibling page.tsx there is a same-specificity conflict Next refuses to build).
  it("scaffolds the admin shell: catch-all route, no React editor entry", () => {
    fs.mkdirSync(path.join(dir, "src", "app"), { recursive: true });
    const result = scaffold(dir);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    ...
    expect(sources.some((src) => src.includes('"meditor/editor"'))).toBe(false);
    expect(sources.some((src) => src.includes('"meditor/element"'))).toBe(true);
  ...
  it("places next.config.ts and meditor.config.json at the project root even with a src/ layout", () => {
    fs.mkdirSync(path.join(dir, "src", "app"), { recursive: true });
    const result = scaffold(dir);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(fs.existsSync(path.join(dir, "next.config.ts"))).toBe(true);
    expect(fs.existsSync(path.join(dir, "meditor.config.json"))).toBe(true);
    expect(fs.existsSync(path.join(dir, "src", "next.config.ts"))).toBe(false);
    expect(result.created).toContain("next.config.ts");
    expect(result.created).toContain("meditor.config.json");
...
describe("applySettings", () => {
  function writeBootstrap(overrides: Record<string, unknown> = {}) {
    fs.writeFileSync(
      path.join(dir, "meditor.config.json"),
      JSON.stringify(
  ...
  it("validates a good config and creates a wired next.config.ts when none exists", () => {
    ...
    expect(fs.readFileSync(path.join(dir, "next.config.ts"), "utf8")).toContain("meditor:admin-route-rewrite");
  });
  ...
  it("detects an already-wired cms.config.ts via the marker comment", () => {
    writeBootstrap();
    fs.writeFileSync(path.join(dir, "cms.config.ts"), "// meditor:bootstrap-wired\nexport {};\n");
    const result = applySettings(dir);
    expect(result.ok).toBe(true);
    if (!result.ok) return;
  ...
  it("flags a legacy cms.config.ts that doesn't read meditor.config.json, without touching it", () => {
    writeBootstrap();
