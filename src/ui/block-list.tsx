...
export function BlockList({
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between gap-2 border-b border-[var(--scms-border)] px-3 py-2">
        <span className="text-xs font-semibold uppercase tracking-wide text-[var(--scms-muted-fg)]">Blocks</span>
      ...
      <ol className="flex-1 overflow-y-auto p-2">
          ...
          return (
            <li
              ...
              className={cn(
                ...
                selected
                  ? "border-[var(--scms-ring)] bg-[var(--scms-muted)]"
                  : "border-transparent hover:bg-[var(--scms-muted)]",
                dragIndex === i && "opacity-40"
              )}
            >
              <GripVertical className="size-4 shrink-0 cursor-grab text-[var(--scms-muted-fg)]" aria-hidden />
              <button
                type="button"
                onClick={() => onSelect(i)}
                className="min-w-0 flex-1 truncate text-left text-sm text-[var(--scms-fg)]"
              >
                <span className="font-medium">{s.slice}</span>
                {typeof s.heading === "string" && (
                  <span className="ml-1 text-[var(--scms-muted-fg)]">— {s.heading.replaceAll("**", "")}</span>
                )}
              </button>
        ...
        {slices.length === 0 && (
          <li className="px-2 py-4 text-center text-sm text-[var(--scms-muted-fg)]">
