 ...
 *  (image-picker-field.tsx). The package MUST NOT import an icon library
 *  itself (it can't depend on whichever one the host ships — `@carbon/icons-react`
 *  here, something else elsewhere); absent → the "icon" control degrades to a
 *  plain text input, same degrade contract as `media`. */
  ...
  return (
    <div className="space-y-2">
      ...
      {icons && (
        <dialog
          ref={dialogRef}
          className="m-auto w-[min(90vw,480px)] rounded-lg border border-[var(--scms-border)] bg-[var(--scms-bg)] p-4 text-[var(--scms-fg)] backdrop:bg-black/30"
        >
          <Input
          ...
          />
          {filtered.length === 0 ? (
            <p className="mt-3 text-sm text-[var(--scms-muted-fg)]">No icons match “{query}”.</p>
          ) : (
