...
const STYLE_ID = "scms-preview-bridge-style";

const STYLE = `
  [data-scms-index]{cursor:pointer}
  [data-scms-index]:hover{outline:2px dashed color-mix(in oklab, var(--scms-primary, #6366f1) 60%, transparent);outline-offset:-2px}
  [data-scms-selected]{outline:2px solid var(--scms-primary, #6366f1) !important;outline-offset:-2px}
  [contenteditable="true"]{outline:2px solid var(--scms-primary, #6366f1);outline-offset:2px;cursor:text}
`;
...
export function initPreviewBridge(): () => void {
  ...
  const wrapperIndex = (node: EventTarget | null): number | null => {
    const el = (node as Element | null)?.closest?.("[data-scms-index]");
    return el ? Number(el.getAttribute("data-scms-index")) : null;
  };
  const mark = (index: number) => {
    document.querySelectorAll("[data-scms-index]").forEach((el) => {
      el.toggleAttribute("data-scms-selected", Number(el.getAttribute("data-scms-index")) === index);
    });
  };
  ...
  const onClick = (e: MouseEvent) => {
    ...
    window.parent?.postMessage({ __scms: true, type: "select", index }, origin);
  ...
  const onDblClick = (e: MouseEvent) => {
    ...
    el.dataset.scmsBefore = el.textContent ?? "";
    el.focus();
    const onBlur = () => {
      el.removeAttribute("contenteditable");
      const before = el.dataset.scmsBefore ?? "";
      const after = el.textContent ?? "";
      delete el.dataset.scmsBefore;
      el.removeEventListener("blur", onBlur);
      if (after !== before) {
        window.parent?.postMessage(
          { __scms: true, type: "edit", index: wrapperIndex(el), before, after },
          origin
        );
      }
    };
  ...
  const onMessage = (e: MessageEvent) => {
    ...
    if ((d.type === "select" || d.type === "scrollTo") && typeof d.index === "number") {
      ...
      document
        .querySelector(`[data-scms-index="${d.index}"]`)
