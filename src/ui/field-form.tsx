...
function YamlField({
  ...
  return (
    <div>
      <Label>
        {label} <span className="text-[var(--scms-muted-fg)]">(YAML)</span>
      </Label>
      <Textarea
      ...
      />
      {error && <span className="mt-1 block text-xs text-[var(--scms-destructive)]">{error}</span>}
    </div>
  );
}
...
>): ReactNode {
  ...
  if (control === "color") {
    ...
    // resolves through a host-defined `--scms-swatch-<token>` custom property
    // (dandelion-site/src/app/globals.css is the reference implementation). A
    // token with no matching variable falls back to `transparent`, not a crash.
    const current = typeof value === "string" ? value : "";
    const options = def?.options ?? [];
    return (
      <div>
          ...
          {options.map((o) => (
            <button
              ...
              className={cn(
                ...
                current === o
                  ? "border-[var(--scms-ring)] shadow-[0_0_0_2px_var(--scms-bg)]"
                  : "border-[var(--scms-border)]"
              )}
              style={{ backgroundColor: `var(--scms-swatch-${o}, transparent)` }}
            />
          ))}
        </div>
      </div>
  ...
  if (control === "boolean") {
    return (
      <label className="flex items-center gap-2 text-sm text-[var(--scms-fg)]">
        <input
          type="checkbox"
          className="size-4 accent-[var(--scms-primary)]"
          checked={value === true}
    ...
    return (
      <div>
        <Label>
          {label}
          {control === "richtext" && (
            <span className="ml-1 font-normal text-[var(--scms-muted-fg)]">(**bold** supported)</span>
          )}
        </Label>
        <Textarea
          className="min-h-20"
  ...
  if (control === "image" || control === "media") {
    ...
    // top-level array. Arrays of `{src,alt}` (logoWall.logos) are handled by
    // "slot" now, not here.
    ...
    <div>
      ...
      <div className="space-y-3">
        {value.map((item, i) => {
          ...
          return (
            <div key={ids[i]} className="space-y-3 rounded-md border border-[var(--scms-border)] p-3">
              <div className="flex items-center justify-between gap-2">
                <span className="truncate text-sm font-medium text-[var(--scms-fg)]">{title}</span>
...
export function FieldForm({
  ...
  return (
    <div className="space-y-3">
      ...
      {keys.length === 0 && (
        <p className="text-sm text-[var(--scms-muted-fg)]">No fields. Use “Edit as YAML” to add some.</p>
      )}
      {keys.map((key) => (
        <FieldRow
          key={key}
...
function BlockHeader({
  ...
  return (
    <div className="flex items-center justify-between">
      <span className="rounded bg-[var(--scms-muted)] px-2 py-0.5 font-mono text-xs text-[var(--scms-fg)]">
        {name}
      </span>
