    ...
    >
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          <Search
            className="pointer-events-none absolute left-2 top-1/2 size-4 -translate-y-1/2 text-[var(--scms-muted-fg)]"
            aria-hidden
          />
          <Input
            className="pl-8"
      ...
      {assets.length === 0 ? (
        <div className="flex flex-col items-center gap-2 py-10 text-sm text-[var(--scms-muted-fg)]">
            ...
            <li key={a.id}>
              {mode === "picker" ? (
                <button
                  type="button"
                  onClick={() => onSelect?.(a)}
                  className="block w-full overflow-hidden rounded-md border border-[var(--scms-border)] text-left hover:border-[var(--scms-ring)]"
                >
                  <Thumb asset={a} />
                </button>
              ) : (
                <div className="overflow-hidden rounded-md border border-[var(--scms-border)]">
                  <Thumb asset={a} />
...
function Thumb({ asset }: Readonly<{ asset: MediaAsset }>) {
  return (
    <div>
      ...
      <div className="truncate px-1.5 py-1 text-xs text-[var(--scms-fg)]">{asset.name}</div>
      <div className="px-1.5 pb-1 text-[10px] text-[var(--scms-muted-fg)]">{humanSize(asset.size)}</div>
    </div>
  );
}
