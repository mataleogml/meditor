 ...
 *  host's Server Actions + upload route. Absent → ImagePickerField degrades
 *  to a plain text input with no "Browse library" button. */
  ...
  return (
    <div className="space-y-2">
      {value ? (
        <div className="space-y-2">
          ...
          <img
            ...
            alt=""
            loading="lazy"
            className="h-24 w-full rounded-md border border-[var(--scms-border)] bg-[var(--scms-bg)] object-contain p-1"
          />
        ...
        <button
          type="button"
          onClick={openLibrary}
          className="flex h-24 w-full items-center justify-center rounded-md border border-dashed border-[var(--scms-border)] text-sm text-[var(--scms-fg)] opacity-70 transition hover:border-[var(--scms-fg)] hover:opacity-100"
        >
          + Add image
        </button>
      ) : (
      ...
      {media && (
        <dialog
          ref={dialogRef}
          className="m-auto w-[min(90vw,720px)] rounded-lg border border-[var(--scms-border)] bg-[var(--scms-bg)] p-4 text-[var(--scms-fg)] backdrop:bg-black/30"
        >
          <MediaGrid
            assets={filtered}
            mode="picker"
