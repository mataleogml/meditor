...
beforeEach(() => {
  dir = fs.mkdtempSync(path.join(os.tmpdir(), "meditor-settings-"));
...
describe("createFsSettingsAdapter", () => {
  it("round-trips bootstrap fields atomically (no leftover tmp file)", () => {
    const file = path.join(dir, "meditor.config.json");
...
describe("createSettingsStore", () => {
  it("sites its adapter in a private .meditor/ dir beside the Pages content dir", () => {
    createSettingsStore(makeConfig());
    expect(fs.existsSync(path.join(dir, ".meditor"))).toBe(false); // not created until first write
  });
  ...
  it("saveDraft then publish round-trips runtime settings, never touching the Pages dir", async () => {
    const store = createSettingsStore(makeConfig());
    const draft = { brand: { name: "Dandelion" }, seo: { description: "A site" } };
    ...
    expect(fs.existsSync(path.join(dir, ".meditor", "settings.md"))).toBe(true);
  });
