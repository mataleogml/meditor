...
import { currentAdminRoute, meditorProxyRewrite, previewPathFor } from "./proxy";

let dir: string;
let file: string;

beforeEach(() => {
  dir = fs.mkdtempSync(path.join(os.tmpdir(), "meditor-proxy-"));
  file = path.join(dir, "meditor.config.json");
});
...
describe("meditorProxyRewrite", () => {
  it("maps a renamed admin route's prefix to /editor", () => {
    fs.writeFileSync(file, JSON.stringify({ adminRoute: "backoffice" }));
    expect(meditorProxyRewrite("/backoffice", { configFile: file })).toBe("/editor");
    expect(meditorProxyRewrite("/backoffice/preview", { configFile: file })).toBe("/editor/preview");
    expect(meditorProxyRewrite(new URL("http://x/backoffice/pages/home"), { configFile: file })).toBe(
      "/editor/pages/home"
    );
  });
  ...
  it("returns null for a path outside the admin route", () => {
    fs.writeFileSync(file, JSON.stringify({ adminRoute: "backoffice" }));
    expect(meditorProxyRewrite("/blog/post-1", { configFile: file })).toBeNull();
    // Prefix collision, not a real path segment match.
    expect(meditorProxyRewrite("/backofficexyz", { configFile: file })).toBeNull();
  });

  it("returns null when adminRoute is still the default \"editor\" (nothing to rewrite)", () => {
    expect(meditorProxyRewrite("/editor/pages/home", { configFile: file })).toBeNull();
  });
});
