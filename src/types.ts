...
export type FieldDef = {
  type?:
    ...
    | "color" // a token choice rendered as a real swatch, not the word — reuses `options`; each token resolves through a host-defined `--scms-swatch-<token>` CSS custom property (the package can't know the host's palette any more than it can its icon library)
...
export interface ContentAdapter {
   ...
   *  site something beside it (the `.meditor/` settings dir, a collection's
