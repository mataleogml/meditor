...
import { buildTemplates, MEDITOR_BOOTSTRAP_MARKER, MEDITOR_REWRITE_MARKER } from "./templates/init";
import { createFsSettingsAdapter, type SiteSettingsBootstrap } from "./settings";
...
/**
 * Core of `meditor init`: detect the App Router base dir, write every
...
export function scaffold(cwd: string, opts: { force?: boolean } = {}): ScaffoldResult {
  ...
  if (!baseDir) {
    return {
      ...
      error:
        ...
        "Run `meditor init` from the root of a Next.js App Router project.",
    };
  }
  ...
  for (const [relPath, content] of Object.entries(templates)) {
    // A "/"-prefixed key (next.config.ts, meditor.config.json) always lands at
...
// shared export, same call settings.element.ts's `validateAdminRoute` makes:
// this is a Node-side CLI concern, that one a UI concern, and duplicating one
// regex + one Set is cheaper than coupling the two layers.
...
/**
 * Core of `meditor apply-settings`: reconciles the host's next.config /
 * cms.config with meditor.config.json's bootstrap tier (spec §5's "Admin-route
 ...
 * meditor.config.json and reads `bootstrap.adminRoute` directly — no codegen,
...
export function applySettings(cwd: string): ApplySettingsResult {
  const bootstrapFile = path.join(cwd, "meditor.config.json");
  const bootstrap = createFsSettingsAdapter({ file: bootstrapFile }).readBootstrap();

  const error = validateBootstrap(bootstrap);
  if (error) return { ok: false, error: `invalid meditor.config.json: ${error}` };
  ...
  if (!foundNextConfig) {
    ...
    nextConfig = "created";
  } else if (fs.readFileSync(foundNextConfig, "utf8").includes(MEDITOR_REWRITE_MARKER)) {
    nextConfig = "already-wired";
  } else {
    nextConfig = "needs-manual-update";
    notes.push(
      `${path.relative(cwd, foundNextConfig)} doesn't read meditor.config.json yet. Add:\n\n` +
        `    import bootstrap from "./meditor.config.json";\n` +
        "    // inside rewrites():\n" +
  ...
  if (!foundCmsConfig) {
    cmsConfig = "not-found";
    notes.push("no cms.config.ts found — run `meditor init` first.");
  } else if (fs.readFileSync(foundCmsConfig, "utf8").includes(MEDITOR_BOOTSTRAP_MARKER)) {
    cmsConfig = "already-wired";
  } else {
    cmsConfig = "needs-manual-update";
    notes.push(
      `${path.relative(cwd, foundCmsConfig)} doesn't read locales/defaultLocale from meditor.config.json yet. Add:\n\n` +
        '    import { createFsSettingsAdapter } from "meditor";\n' +
        '    const bootstrap = createFsSettingsAdapter({ file: path.join(process.cwd(), "meditor.config.json") }).readBootstrap();\n' +
...
// ---------------------------------------------------------------------------
// `meditor review` — deterministic content-review brief (see spec-A). Same
...
/** Slice names with a `meditor/seo` seoRegistry entry, scraped the same
...
function checkTitleAndDescription(meta: Record<string, unknown>): CheckResult[] {
  ...
  // seo.config.ts's titleTemplate ("%s | Dandelion Payments") adds ~19 chars
  // on top of a page's own <title> — 60 is the recommended ceiling BEFORE
  // that suffix, per SERP truncation guidance (see spec-A §3).
...
function checkRobotsCrawlable(meta: Record<string, unknown>): CheckResult {
  // meditor/seo's buildRobots doesn't special-case noindex pages — they stay
...
/**
 * Core of `meditor review [slug] [--base <ref>]`. No slug: one brief per
...
export function openOrUpdateContentPr(cwd: string, brief: ReviewBrief): PrResult {
  ...
  const bodyFile = path.join(os.tmpdir(), `meditor-review-${brief.slug}-${Date.now()}.md`);
  try {
...
/**
 * `meditor review --update-pr <n> --body-file <path>` — the seam for the
...
function printReviewReport(result: ReviewResult, opts: { json?: boolean } = {}): void {
  ...
  if (!result.ok) {
    console.error(`meditor review: ${result.error}`);
    process.exitCode = 1;
    return;
  }
  if (!result.briefs.length) {
    console.log("meditor review: no changed files under src/content against the base ref.");
    return;
  }
...
1. Add meditor's editor styles near your Tailwind entry (usually ${appDir}/globals.css):

     @import "meditor/theme.css";
     @source "../node_modules/meditor/dist"; /* Tailwind v4: adjust the relative path to your CSS file's location */

   Then map the --scms-* tokens it defines onto your own design tokens (adjust
...
function printHelp(): void {
  console.log(`meditor — scaffolder CLI

Usage:
  npx meditor init [--force]
  npx meditor apply-settings
  npx meditor review [slug] [--base <ref>] [--json] [--pr]
  npx meditor review --update-pr <number> --body-file <path>

  init             Scaffold meditor's editor wiring into the current project
                   (default command). Never overwrites an existing file
                   unless --force is given.

  apply-settings   Reconcile the host's next.config/cms.config with
                   meditor.config.json's adminRoute/locales/defaultLocale
...
function printApplySettingsReport(result: ApplySettingsResult): void {
  if (!result.ok) {
    console.error(`meditor apply-settings: ${result.error}`);
    process.exitCode = 1;
    return;
  }
  console.log(
    `meditor apply-settings: bootstrap OK (adminRoute="${result.bootstrap.adminRoute}", ` +
...
function runReviewCommand(args: string[]): void {
  ...
  if (opts.updatePr !== undefined) {
    ...
    if (!Number.isInteger(prNumber) || !opts.bodyFile) {
      console.error("meditor review: --update-pr <number> requires a numeric PR number and --body-file <path>");
      process.exitCode = 1;
      return;
    }
    ...
    console.log(`meditor review: PR #${prNumber} ${result.action}.`);
    return;
  }
  ...
  if (opts.pr) {
    for (const brief of result.briefs) {
      ...
      if (!prResult.ok) {
        console.error(`meditor review: PR step failed for "${brief.slug}": ${prResult.error}`);
        process.exitCode = 1;
      } else {
        console.log(`meditor review: PR ${prResult.action} for "${brief.slug}"${prResult.url ? ` (${prResult.url})` : ""}.`);
...
function main(): void {
  ...
  if (command !== "init") {
    console.error(`meditor: unknown command "${command}" (only "init", "apply-settings", and "review" are supported)`);
    process.exitCode = 1;
    return;
  }
  ...
  if (!result.ok) {
    console.error(`meditor init: ${result.error}`);
    process.exitCode = 1;
    return;
  }

  const { created, skipped, baseDir } = result;
  console.log(`meditor init: ${created.length} file(s) created, ${skipped.length} skipped (already exist).`);
  if (created.length) {
    console.log("\nCreated:");
