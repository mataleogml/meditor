 ...
 * `meditor.config.json` per request (mtime-cached below) and rewrite
 ...
 * under static export. Those hosts stay on the Wave-4 `meditor apply-settings`
 ...
 * import { meditorProxyRewrite } from "meditor/proxy";
 *
 * export function proxy(request: NextRequest) {
 *   const rewritten = meditorProxyRewrite(request.nextUrl);
...
const DEFAULT_CONFIG_FILE = () => path.join(process.cwd(), "meditor.config.json");
...
/** Current `adminRoute` from `meditor.config.json`, mtime-cached so a steady
 ...
 */
export function meditorProxyRewrite(url: URL | string, opts?: { configFile?: string }): string | null {
