...
export function makeActions(config: CmsConfig) {
  ...
  async function guard(action: AuthAction, slug?: string) {
    if (!(await auth.authorize({ action, slug }))) throw new Error("meditor: unauthorized");
  }
  ...
  return {
    ...
    async createPage(title: string, locale?: string): Promise<string> {
      ...
      if (!slug) throw new Error("meditor: page name is required");
    ...
    async renamePage(slug: string, newSlug: string): Promise<RenameResult> {
      ...
      if (!target) throw new Error("meditor: new slug is required");
      try {
    ...
    async listMedia() {
      await guard("read");
      if (!config.mediaAdapter) throw new Error("meditor: no mediaAdapter configured");
 ...
 *  (see `src/app/editor/*`), not the real `makeActions()` object, so this
 *  must list only what's used, not the full action registry.
 *
 ...
 *  stub before it typechecks. `MeditorClient` (see api/client.ts) still
