...
export async function processUpload(input: { name: string; buffer: Buffer }): Promise<PreparedFile> {
  if (input.buffer.length > MAX_UPLOAD_BYTES) {
    throw new Error(`meditor: upload exceeds ${Math.floor(MAX_UPLOAD_BYTES / (1024 * 1024))}MB limit`);
  }
  ...
  if (meta.format === "svg") {
    if (SVG_SCRIPT_RE.test(input.buffer.toString("utf8"))) {
      throw new Error("meditor: SVG upload rejected — contains script-like content");
    }
    mime = "image/svg+xml";
    buffer = input.buffer;
  } else if (
