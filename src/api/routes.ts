 ...
 * `createMeditorClient` (./client) turns it back into the same object shape the
 * UI already consumes.
 *
 ...
 */
export interface MeditorApiOptions {
  /** Path prefix to strip before matching, e.g. "/api/meditor". Inferred from
 ...
 *
 * ```ts
 * // app/api/meditor/[...path]/route.ts
 * import { createMeditorApi } from "meditor/api";
 * import { cmsConfig } from "@/cms.config";
 * export const { GET, POST, PUT, DELETE } = createMeditorApi(cmsConfig, {
 *   basePath: "/api/meditor",
 * });
 * ```
 */
export function createMeditorApi(config: CmsConfig, options: MeditorApiOptions = {}) {
